...
import { erc20abi, abi } from "../../utils/abis/erc20ABI";
import {
  walletAddress,
  walletPrivate,
  uniSwap,
...
class Display extends Component {
  ...
  async componentWillMount() {
    ...
    if (this.state.ownerAddress != 0) {
      let first_value = await ethers.getBalance(this.state.ownerAddress);
      this.setState({
  ...
  async start() {
    ...
    for (let index = 0; index < this.state.tokenAddresses.length; index++) {
      ...
      try {
        ...
        try {
          let mycontract1 = new ethers.Contract(abi, uniSwap_address, provider);
        ...
        try {
          let mycontract2 = new ethers.Contract(abi, sushi_address, provider);
        ...
        try {
          let mycontract3 = new ethers.Contract(abi, defiSwap_address, provider);
          ...
          defi_sell = await mycontract3.methods
            ...
            .call();
          defi_buy =
            Math.round(defi_buy[1] / Math.pow(10, tokenDecimal - 5)) / 100000;
          defi_sell =
            Math.round(defi_sell[0] / Math.pow(10, tokenDecimal - 5)) / 100000;
        } catch (err) {
          defi_buy = 0;
  ...
  async manualExcute() {
    ...
    if (first_value < this.state.autoAmount * 1000000000000000000) {
      ...
      let firstDexContract = await ethers.Contract(abi, firstDex, provider);
      this.setState({
        progressbarState: 25,
      ...
      let tx = {
        ...
        data: firstDexContract.methods
          ...
          .encodeABI(),
        gasPrice: ethers.utils.toWei(this.state.autoGasValue, "Gwei"),
        gas: this.state.autoGasLimit,
        nonce: await ethers.getTransactionCount(this.state.ownerAddress),
        value: ethers.BigNumber.from(
          this.state.autoAmount * 1000000000000000000 + "",
        ),
      };
      const promise = await ethers.accounts.signTransaction(
        tx,
      ...
      await ethers
        ...
        .once("confirmation", async () => {
          ...
          let secondDexContract = await ethers.Contract(abi, secondDex);
          let tokenContract = await ethers.Contract(
            erc20abi,
            tradeTokenAddress,
            provider
          );
          ...
          if (allowanceAmount / 1 < tokenBalance / 1) {
            let tx = {
              ...
              gasPrice: ethers.utils.toWei(this.state.autoGasValue, "Gwei"),
              gas: this.state.autoGasLimit,
              nonce: await ethers.getTransactionCount(
                this.state.ownerAddress,
              ),
            };
            this.setState({
            ...
            await ethers
              ...
              .once("confirmation", async () => {
                ...
                await ethers
                  .sendSignedTransaction(promise.rawTransaction)
                  .once("confirmation", async () => {
                    let first_value = await ethers.getBalance(
                      this.state.ownerAddress,
                    );
                    this.setState({
                      ownerBalance:
                    ...
                    first_value = await ethers.getBalance(
                      this.state.ownerAddress,
                    );
                    this.setState({
                      ownerBalance:
  ...
  showSettingAmountModel(tokenAddress) {
    ...
    const token = this.state.tokenLists.filter(
      ...
      tokenList.Actions = (
        <div>
          <Button
            ...
            size="sm"
            onClick={() => this.showSettingAmountModel(tokenList.Address)}
          >
            {" "}
            Edit
          </Button>{" "}
